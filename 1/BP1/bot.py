...
def NOTIFIED_ACCEPT_GROUP_INVITATION(op):
    try:
        sendMessage("ยินดีต้อน" + op.param1, client.getContact(op.param2).displayName + "/nBOT BY SATAN")
...
def NOTIFIED_KICKOUT_FROM_GROUP(op):
    try:
				...
				sendMessage(op.param1, client.getContact(op.param2).displayName + ", ได้ทำการลบสมาชิกในกลุ่ม")				
    except Exception as e:
        print e
...
def NOTIFIED_UPDATE_GROUP(op):
    try:
                sendMessage(op.param1, client.getContact(op.param2).displayName + ", ผู้ใช้นี้ได้ทำการเปลี่ยนแปลงการตั่งค่ากลุ่ม")
                client.kickoutFromGroup(op.param1,[op.param2])
    except Exception as e:
        print e
